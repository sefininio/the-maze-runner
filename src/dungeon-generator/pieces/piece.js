...
class Piece {
    ...
    add_perimeter(p_from, p_to, facing) {
        utils.iter_range(p_from, p_to, pos => {
    ...
    intersects(piece) {
        return utils.intersects(this.position, this.size, piece.position, piece.size);
    }

    add_piece(piece, position = null) {
        if (utils.array_test(this.children, c => c.id === piece.id)) {
            return;
        }
        piece.parent = this;
        if (position) {
